...
ALTER TABLE Material MODIFY COLUMN Title VARCHAR(255);
...
SELECT 
    ...
    m.Title,
    CASE 
        WHEN b.Borrow_ID IS NOT NULL THEN 'Not Available'
        ELSE 'Available'
    END AS Status
FROM 
    Material m
LEFT JOIN 
    Borrow b ON m.Material_ID = b.Material_ID AND b.Return_Date IS NULL;
...
-- 4 How many materials has the author Lucas Piki wripen?
...
-- 5 How many materials were wripen by two or more authors?
...
SELECT 
    Genre.Name AS Genre, 
    (SELECT COUNT(*) FROM Borrow WHERE Borrow.Material_ID 
    IN (SELECT Material_ID FROM Material WHERE Material.Genre_ID = Genre.Genre_ID))
    AS Total_Borrowed
FROM 
    Genre
ORDER BY 
...
SELECT COUNT(DISTINCT Material_ID) AS Total_Borrowed_Materials
FROM Borrow
...
SELECT m.*, b.Borrow_ID, b.Member_ID, b.Staff_ID, b.Borrow_Date, b.Due_Date, b.Return_Date
FROM Material m
JOIN Borrow b ON m.Material_ID = b.Material_ID
...
INSERT INTO Author (Author_ID,Name, Birth_Date, Nationality) VALUES ('21','Lucas Luke', '2020-08-10', 'American'); 
 
# Insert into Material table 
 
INSERT INTO Material (Material_ID,Title, Publication_Date, Catalog_ID, Genre_ID) VALUES 
(32,'New book', '2020-08-01', (SELECT Catalog_ID FROM Catalog WHERE Name = 'E-Books'), (SELECT Genre_ID FROM Genre WHERE Name = 'Mystery & Thriller')); 
...
WHERE 
  ...
  CREATE EVENT DailyAlert
ON SCHEDULE EVERY 1 DAY
STARTS CURRENT_TIMESTAMP
DO 
CALL AlertStaffAboutOverdue();
...
-- 2. Automa<cally deactivate the membership based on the member’s overdue occurrence (>= three times). And reactivate the membership once the member pays the overdue fee.


 DELIMITER //
CREATE PROCEDURE ManageMembershipStatus()
BEGIN
    -- Deactivate memberships with three or more overdue occurrences
    UPDATE Member m
    JOIN (
        SELECT Member_ID
        FROM Borrow
        ...
        GROUP BY Member_ID
        HAVING COUNT(*) >= 3
    ) AS overdue ON m.Member_ID = overdue.Member_ID
    SET m.Membership_Status = 'Inactive';

    -- Reactivate memberships when the overdue fee is paid
    UPDATE Member m
    JOIN Borrow b ON m.Member_ID = b.Member_ID
    SET m.Membership_Status = 'Active'
    WHERE b.Return_Date IS NOT NULL
    AND b.Overdue_Fee_Paid = 1;
END//
